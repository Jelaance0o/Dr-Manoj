...
const PricingCard = ({ plan, className = "", style }) => (
  <div
    ...
    className={`relative flex flex-col rounded-2xl border p-6 sm:p-8 shadow-card ${
      plan.highlighted
        ? "border-primary bg-white shadow-lifted lg:scale-[1.03]"
  ...
  return (
    <section id="fees" className="bg-paper py-14 lg:py-18">
      <div className="container-px">
        ...
        <div
          ref={headRef}
          className={`mx-auto max-w-2xl text-center ${revealClasses(
            headVisible,
          )}`}
        >
          ...
          <p className="mt-4 text-ink-soft">
            Quality holistic healthcare made accessible to families across Badi
            Sarwan and neighboring towns. No hidden diagnostic charges.
          </p>
        </div>

        {/* Desktop / Tablet: animation kept */}
        <div
          ref={gridRef}
          className="mt-12 hidden gap-7 sm:grid sm:grid-cols-3 sm:items-stretch lg:mt-14"
        >
          {feePlans.map((plan, index) => (
            <PricingCard
              ...
              className={`transition-transform duration-300 hover:-translate-y-1 hover:scale-[1.01] ${revealClasses(
                gridVisible,
              )}`}
            />
          ))}
        ...
        {/* Mobile: completely static — NO animation */}
